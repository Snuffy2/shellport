...
The Docker Compose example above mounts `./config` as a writable configuration directory. By default, ShellPort loads `/etc/shellport/shellport.conf.json`, so `SHELLPORT_CONFIG` is only needed when you want to override the config path. Start from [shellport.conf.example.json](shellport.conf.example.json) when creating your own configuration.
