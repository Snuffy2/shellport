...
func (d connectionDebugDetails) fields() string {
	...
	if d.User != "" {
		fields = append(fields, "user="+d.User)
	}
	if d.Address != "" {
		fields = append(fields, "address="+d.Address)
	}
	if d.Network != "" {
		fields = append(fields, "network="+d.Network)
	}
	if d.AuthMethod != "" {
		fields = append(fields, "auth_method="+d.AuthMethod)
	}
	if d.PresetID != "" {
		fields = append(fields, "preset_id="+d.PresetID)
	}
